...
var hiss_CCInterp = $hx_exports["hiss"]["CCInterp"] = function(printFunction) {
	...
	this.importSpecialForm(function(args,env,cc) {
		_g(global1,args,env,cc);
	},"setlocal");
	var _g1 = $bind(this,this.set);
	var global2 = true;
	this.importSpecialForm(function(args,env,cc) {
		_g1(global2,args,env,cc);
	},"defvar");
	var _g2 = $bind(this,this.setCallable);
	var isMacro = false;
	this.importSpecialForm(function(args,env,cc) {
		_g2(isMacro,args,env,cc);
	},"defun");
	var _g3 = $bind(this,this.setCallable);
	var isMacro1 = true;
	this.importSpecialForm(function(args,env,cc) {
		_g3(isMacro1,args,env,cc);
	},"defmacro");
	this.importSpecialForm($bind(this,this._if),"if");
	var _g4 = $bind(this,this.lambda);
	var isMacro2 = false;
	this.importSpecialForm(function(args,env,cc) {
		_g4(isMacro2,args,env,cc);
	},"lambda");
	...
	this.importCCFunction(function(args,env,cc) {
		hiss_VariadicFunctions.numCompare(type,args,env,cc);
	},"<");
	var type1 = hiss_Comparison.LesserEqual;
	this.importCCFunction(function(args,env,cc) {
		hiss_VariadicFunctions.numCompare(type1,args,env,cc);
	},"<=");
	var type2 = hiss_Comparison.Greater;
	this.importCCFunction(function(args,env,cc) {
		hiss_VariadicFunctions.numCompare(type2,args,env,cc);
	},">");
	var type3 = hiss_Comparison.GreaterEqual;
	this.importCCFunction(function(args,env,cc) {
		hiss_VariadicFunctions.numCompare(type3,args,env,cc);
	},">=");
	var type4 = hiss_Comparison.Equal;
	this.importCCFunction(function(args,env,cc) {
		hiss_VariadicFunctions.numCompare(type4,args,env,cc);
	},"=");
...
hiss_CCInterp.prototype = {
	...
	,importClass: function(clazz,name,methodNameFunction) {
			...
			if(Type.typeof(fieldValue[0])._hx_index == 5) {
				...
				hiss_HissTools.put(this.globals,translatedName,hiss_HValue.Function((function(fieldValue) {
					return function(args,env,cc) {
						...
						cc(hiss_HissTools.toHValue(fieldValue[0].apply(instance,args1)));
					};
		...
		while(_g < _g1.length) {
			...
			++_g;
			var fieldValue1 = [Reflect.getProperty(clazz,classField)];
			if(Type.typeof(fieldValue1[0])._hx_index == 5) {
	...
	,importFunction: function(func,name,keepArgsWrapped,args) {
		...
		var tmp = hiss_HValue.Function(function(args,env,cc) {
			...
			cc(hiss_HissTools.toHValue(func.apply(null,args1)));
		},name,args);
	...
	,envWithReturn: function(env,cc,called) {
		...
		var value = hiss_HValue.Function(function(args,env,cc) {
			called.b = true;
	...
	,setCallable: function(isMacro,args,env,cc) {
		...
		this.lambda(isMacro,hiss_HissTools.rest(args),env,function(fun) {
			_gthis.set(true,hiss_HissTools.cons(hiss_HissTools.first(args),hiss_HValue.List([fun])),env,cc);
	...
	,getVar: function(name,env,cc) {
		...
		while(_g < stackFrames.length) {
			...
			if(Object.prototype.hasOwnProperty.call(frameDict.h,name1)) {
				v = frameDict.h[name1];
				break;
			}
		}
		...
		if(v != null) {
			tmp = v;
		} else if(Object.prototype.hasOwnProperty.call(g.h,name1)) {
			tmp = g.h[name1];
		} else {
			throw haxe_Exception.thrown("" + name1 + " is undefined");
		}
		cc(tmp);
	}
	...
	,loop: function(args,env,cc) {
		...
		this.evalAll(hiss_HissTools.alternates(bindings,false),env,function(firstValues) {
			...
			while(true) {
				...
				_gthis.internalEval(hiss_HissTools.cons(hiss_HValue.Symbol("begin"),body),hiss_HissTools.extend(env,hiss_HissTools.destructuringBind(names,hiss_HissTools.cons(hiss_HValue.SpecialForm(recur,"recur"),values))),function(value) {
					result = value;
				});
				if(!recurCalled) {
					break;
	...
	,bound: function(args,env,cc) {
		...
		while(_g < stackFrames.length) {
			var frameDict = hiss_HissTools.toDict(stackFrames[_g++]);
			if(Object.prototype.hasOwnProperty.call(frameDict.h,name)) {
				cc(hiss_HValue.T);
			}
		}
		cc(Object.prototype.hasOwnProperty.call(g.h,name) ? hiss_HValue.T : hiss_HValue.Nil);
	}
	...
	,internalEval: function(exp,env,cc) {
		...
		case 2:
			...
			if(v != null) {
				tmp = v;
			} else if(Object.prototype.hasOwnProperty.call(g.h,name)) {
				tmp = g.h[name];
			} else {
				throw haxe_Exception.thrown("" + name + " is undefined");
			}
			cc1(tmp);
			break;
		case 3:
		...
		case 7:
			...
			this.internalEval(hiss_HissTools.first(exp),env,function(callable) {
				...
				case 11:
					...
					break;
				default:
					throw haxe_Exception.thrown("Hiss cannot call " + Std.string(callable) + " from " + hiss_HissTools.toPrint(hiss_HissTools.first(exp)));
				}
			});
			break;
		case 9:
		...
		case 15:
			...
			break;
		default:
			throw haxe_Exception.thrown("Cannot evaluate " + Std.string(exp) + " yet");
		}
	}
	...
	,toNativeFunction: function(fun) {
		...
		if(fun._hx_index == 9) {
			...
			if(_g != null && _g.length == 0) {
			...
			} else if(_g != null && _g.length == 1) {
				return function(arg1) {
					var val = null;
					_gthis.funcall(false,hiss_HValue.List([fun,hiss_HValue.Quote(hiss_HissTools.toHValue(arg1))]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),function(_val) {
						val = _val;
					});
					return hiss_HissTools.value(val,null,true);
				};
			} else if(_g != null && _g.length == 2) {
				return function(arg1,arg2) {
					var val = null;
					_gthis.funcall(false,hiss_HValue.List([fun,hiss_HValue.Quote(hiss_HissTools.toHValue(arg1)),hiss_HValue.Quote(hiss_HissTools.toHValue(arg2))]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),function(_val) {
						val = _val;
					});
					return hiss_HissTools.value(val,null,true);
				};
			} else if(_g != null && _g.length == 3) {
				return function(arg1,arg2,arg3) {
					var val = null;
					_gthis.funcall(false,hiss_HValue.List([fun,hiss_HValue.Quote(hiss_HissTools.toHValue(arg1)),hiss_HValue.Quote(hiss_HissTools.toHValue(arg2)),hiss_HValue.Quote(hiss_HissTools.toHValue(arg3))]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),function(_val) {
						val = _val;
					});
					return hiss_HissTools.value(val,null,true);
				};
			} else if(_g != null && _g.length == 4) {
				return function(arg1,arg2,arg3,arg4) {
					var val = null;
					_gthis.funcall(false,hiss_HValue.List([fun,hiss_HValue.Quote(hiss_HissTools.toHValue(arg1)),hiss_HValue.Quote(hiss_HissTools.toHValue(arg2)),hiss_HValue.Quote(hiss_HissTools.toHValue(arg3)),hiss_HValue.Quote(hiss_HissTools.toHValue(arg4))]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),function(_val) {
						val = _val;
					});
					return hiss_HissTools.value(val,null,true);
				};
			} else if(_g != null && _g.length == 5) {
				return function(arg1,arg2,arg3,arg4,arg5) {
					var val = null;
					_gthis.funcall(false,hiss_HValue.List([fun,hiss_HValue.Quote(hiss_HissTools.toHValue(arg1)),hiss_HValue.Quote(hiss_HissTools.toHValue(arg2)),hiss_HValue.Quote(hiss_HissTools.toHValue(arg3)),hiss_HValue.Quote(hiss_HissTools.toHValue(arg4)),hiss_HValue.Quote(hiss_HissTools.toHValue(arg5))]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),function(_val) {
						val = _val;
					});
					return hiss_HissTools.value(val,null,true);
				};
				...
				throw haxe_Exception.thrown("Function has no args specified, cannot be converted");
			} else {
...
var hiss_HissReader = function(interp) {
	...
	this.internalSetMacroString("(",function(start,stream) {
		return _g(terminator,delimiters,start,stream);
	});
...
hiss_HissReader.prototype = {
	...
	,read: function(start,stream) {
		...
		while(_g < _g1.length) {
			...
			if(Object.prototype.hasOwnProperty.call(this.readTable.h,couldBeAMacro)) {
				stream.drop(couldBeAMacro);
				stream.position();
				var expression = null;
				expression = this.callReadFunction(this.readTable.h[couldBeAMacro],couldBeAMacro,stream);
...
hiss_HissTools.toFloat = function(v) {
	if(v._hx_index == 1) {
		...
		throw haxe_Exception.thrown("extraction to `" + "float" + "` failed on `" + Std.string(v) + "`");
	}
};
...
hiss_HissTools.dictExtend = function(dict,extension) {
	var extended = haxe_ds_StringMap.createCopy(hiss_HissTools.toDict(dict).h);
	var pair = haxe_ds_StringMap.kvIterator(hiss_HissTools.toDict(extension).h);
	while(pair.hasNext()) {
		var pair1 = pair.next();
		extended.h[pair1.key] = pair1.value;
	}
...
hiss_HissTools.toPrint = function(v,recursiveCall) {
	...
	case 8:
		if(recursiveCall > hiss_HissTools.recursivePrintDepth) {
			...
			var _g = [];
			var _g1 = haxe_ds_StringMap.kvIterator(v.n.h);
			while(_g1.hasNext()) {
				var _g2 = _g1.next();
				_g.push("" + _g2.key + " => " + hiss_HissTools.toPrint(_g2.value,recursiveCall + 1) + ", ");
			}
...
hiss_VariadicFunctions.add = function(args,env,cc) {
	...
	var _g = 0;
	var _g1 = hiss_HissTools.unwrapList(args);
