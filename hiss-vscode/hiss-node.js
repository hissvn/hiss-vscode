...
var hiss_CCInterp = $hx_exports["hiss"]["CCInterp"] = function(printFunction) {
	...
	this.tempTrace = null;
	this.globals = hiss_HValue.Dict(new haxe_ds_StringMap());
	...
	var f7 = $bind(this,this.iterate);
	this.importSpecialForm(function(args7,env7,cc7) {
		f7(true,true,args7,env7,cc7);
	},"for");
	var f8 = $bind(this,this.iterate);
	this.importSpecialForm(function(args8,env8,cc8) {
		f8(false,true,args8,env8,cc8);
	},"do-for");
	var f9 = $bind(this,this.iterate);
	this.importSpecialForm(function(args9,env9,cc9) {
		f9(true,false,args9,env9,cc9);
	},"map");
	var f10 = $bind(this,this.iterate);
	this.importSpecialForm(function(args10,env10,cc10) {
		f10(false,false,args10,env10,cc10);
	},"do-map");
	this.useBeginFunction($bind(this,this.trBegin));
	var f11 = $bind(this,this.useBeginFunction);
	var bf = $bind(this,this.trBegin);
	this.importFunction(function() {
		f11(bf);
	},"enable-tail-recursion");
	var f12 = $bind(this,this.useBeginFunction);
	var bf1 = $bind(this,this.trBegin);
	this.importFunction(function() {
		f12(bf1);
	},"disable-continuations");
	var f13 = $bind(this,this.useBeginFunction);
	var bf2 = $bind(this,this.begin);
	this.importFunction(function() {
		f13(bf2);
	},"enable-continuations");
	var f14 = $bind(this,this.useBeginFunction);
	...
	this.importFunction(hiss_HissTools.get,"dict-get");
	this.importFunction(hiss_HissTools.put,"dict-set");
	...
	this.importFunction(hiss_HissTools.eq,"eq",hiss_HValue.T,["a","b"]);
	...
	this.importFunction(function(list) {
		return hiss_HissTools.alternates(list,true);
	},"even-alternates",hiss_HValue.T);
	this.importFunction(function(list1) {
		return hiss_HissTools.alternates(list1,false);
	...
	this.importCCFunction(function(args11,env11,cc11) {
		hiss_VariadicFunctions.numCompare(hiss_Comparison.Lesser,args11,env11,cc11);
	},"<");
	this.importCCFunction(function(args12,env12,cc12) {
		hiss_VariadicFunctions.numCompare(hiss_Comparison.LesserEqual,args12,env12,cc12);
	},"<=");
	this.importCCFunction(function(args13,env13,cc13) {
		hiss_VariadicFunctions.numCompare(hiss_Comparison.Greater,args13,env13,cc13);
	},">");
	this.importCCFunction(function(args14,env14,cc14) {
		hiss_VariadicFunctions.numCompare(hiss_Comparison.GreaterEqual,args14,env14,cc14);
	},">=");
	this.importCCFunction(function(args15,env15,cc15) {
		hiss_VariadicFunctions.numCompare(hiss_Comparison.Equal,args15,env15,cc15);
	},"=");
...
hiss_CCInterp.prototype = {
	...
	,disableTrace: function() {
		if(this.tempTrace == null) {
			haxe_Log.trace("Disabling trace",{ fileName : "src/hiss/CCInterp.hx", lineNumber : 43, className : "hiss.CCInterp", methodName : "disableTrace"});
	...
	,enableTrace: function() {
		if(this.tempTrace != null) {
			haxe_Log.trace("Enabling trace",{ fileName : "src/hiss/CCInterp.hx", lineNumber : 51, className : "hiss.CCInterp", methodName : "enableTrace"});
	...
	,importClass: function(clazz,name,methodNameFunction) {
		var _gthis = this;
		...
		while(_g < _g1.length) {
			...
			if(Type.typeof(fieldValue[0])._hx_index == 5) {
				...
				hiss_HissTools.put(this.globals,translatedName,hiss_HValue.Function((function(fieldValue1) {
					return function(args,env,cc) {
						var instance = hiss_HissTools.value(hiss_HissTools.first(args),_gthis);
						var args1 = hiss_HissTools.unwrapList(hiss_HissTools.rest(args),_gthis);
						cc(hiss_HissTools.toHValue(fieldValue1[0].apply(instance,args1)));
						return;
					};
				})(fieldValue),translatedName));
			}
		}
		var _g2 = 0;
		...
		while(_g2 < _g3.length) {
			...
			++_g2;
			var fieldValue2 = [Reflect.getProperty(clazz,classField)];
			if(Type.typeof(fieldValue2[0])._hx_index == 5) {
				var translatedName1 = methodNameFunction(classField);
				hiss_HissTools.put(this.globals,translatedName1,hiss_HValue.Function((function(fieldValue3) {
					return function(args2,env1,cc1) {
						var args3 = hiss_HissTools.unwrapList(args2,_gthis);
						cc1(hiss_HissTools.toHValue(fieldValue3[0].apply(null,args3)));
						return;
					};
				})(fieldValue2),translatedName1));
			}
		}
	}
	...
	,repl: function(useConsoleReader) {
		...
		hiss_HaxeTools.println("Hiss version " + "generators-358* (target: js)");
	...
	,load: function(file) {
		this._load(hiss_HValue.List([hiss_HValue.String(file)]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),hiss_CCInterp.noCC);
	}
	,_load: function(args,env,cc) {
		...
		this.readingProgram = false;
		this.trBegin(exps,env,cc);
	}
	,envWithReturn: function(env,cc,called) {
		var _g = new haxe_ds_StringMap();
		var value = hiss_HValue.Function(function(args,env1,cc1) {
			called.b = true;
			cc1(hiss_HissTools.first(args));
			return;
		},"return");
		if(__map_reserved["return"] != null) {
			_g.setReserved("return",value);
		} else {
			_g.h["return"] = value;
		}
		return hiss_HissTools.extend(env,hiss_HValue.Dict(_g));
	}
	,trBegin: function(exps,env,cc) {
		var returnCalled = new hiss_RefBool();
		env = this.envWithReturn(env,cc,returnCalled);
	...
	,funcall: function(callInline,args,env,cc) {
		this.evalAll(args,env,function(values) {
			(hiss_HissTools.toHFunction(hiss_HissTools.first(values)))(hiss_HissTools.rest(values),callInline ? env : hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),cc);
			return;
		});
	}
	...
	,set: function(global1,args,env,cc) {
		var _gthis = this;
		this.internalEval(hiss_HissTools.second(args),env,function(val) {
			hiss_HissTools.put(global1 ? _gthis.globals : hiss_HissTools.first(env),hiss_HissTools.symbolName(hiss_HissTools.first(args)),val);
			cc(val);
			return;
		});
	}
	...
	,getVar: function(name,env,cc) {
		...
		var name1 = hiss_HissTools.symbolName(name);
		var v = null;
		var _g = 0;
		while(_g < stackFrames.length) {
			var frameDict = hiss_HissTools.toDict(stackFrames[_g++]);
			if(__map_reserved[name1] != null ? frameDict.existsReserved(name1) : frameDict.h.hasOwnProperty(name1)) {
				v = __map_reserved[name1] != null ? frameDict.getReserved(name1) : frameDict.h[name1];
				break;
			}
		}
		var tmp;
		if(v != null) {
			tmp = v;
		} else if(__map_reserved[name1] != null ? g.existsReserved(name1) : g.h.hasOwnProperty(name1)) {
			tmp = __map_reserved[name1] != null ? g.getReserved(name1) : g.h[name1];
		} else {
			throw new js__$Boot_HaxeError("" + name1 + " is undefined");
		}
		cc(tmp);
	}
	,lambda: function(isMacro,args,env,cc,name) {
		...
		var hFun = function(fArgs,innerEnv,fCC) {
			...
			callEnv = hiss_HissTools.extend(callEnv,hiss_HissTools.destructuringBind(params,fArgs));
	...
	,iterate: function(collect,bodyForm,args,env,cc) {
		...
		if(bodyForm) {
			...
			operation = function(innerArgs,innerEnv,cc1) {
				var bodyEnv = hiss_HissTools.destructuringBind(hiss_HissTools.first(args),hiss_HissTools.first(innerArgs));
				var bodyEnv1 = hiss_HissTools.extend(env,bodyEnv);
				var operation1 = hiss_HissTools.cons(hiss_HValue.Symbol("begin"),body);
				_gthis.internalEval(operation1,bodyEnv1,cc1);
				return;
			};
		} else {
		...
		while(value.hasNext()) {
			var tmp = hiss_HValue.List([value.next()]);
			operation(tmp,bodyForm ? env : hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),iterationCC);
		}
	...
	,loop: function(args,env,cc) {
		...
		this.evalAll(hiss_HissTools.alternates(bindings,false),env,function(firstValues) {
			...
			while(true) {
				...
				var tmp2 = hiss_HissTools.destructuringBind(names,tmp1);
	...
	,bound: function(args,env,cc) {
		var stackFrames = hiss_HissTools.toList(env);
		var g = hiss_HissTools.toDict(this.globals);
		var name = hiss_HissTools.symbolName(hiss_HissTools.first(args));
		var _g = 0;
		while(_g < stackFrames.length) {
			var frameDict = hiss_HissTools.toDict(stackFrames[_g++]);
			if(__map_reserved[name] != null ? frameDict.existsReserved(name) : frameDict.h.hasOwnProperty(name)) {
				cc(hiss_HValue.T);
			}
		}
		cc((__map_reserved[name] != null ? g.existsReserved(name) : g.h.hasOwnProperty(name)) ? hiss_HValue.T : hiss_HValue.Nil);
	}
	...
	,callHaxe: function(args,env,cc) {
		...
		cc(hiss_HissTools.toHValue(Reflect.getProperty(caller,hiss_HissTools.toHaxeString(hiss_HissTools.second(args))).apply(caller,hiss_HissTools.unwrapList(hiss_HissTools.third(args),this,keepArgsWrapped))));
	}
	,callCC: function(args,env,cc) {
		...
		var ccHFunction = hiss_HValue.Function(function(innerArgs,innerEnv,innerCC) {
			...
			haxe_Log.trace("calling cc#" + ccId + " with " + hiss_HissTools.toPrint(arg),{ fileName : "src/hiss/CCInterp.hx", lineNumber : 684, className : "hiss.CCInterp", methodName : "callCC"});
			cc(arg);
			return;
		},"cc");
	...
	,internalEval: function(exp,env,cc) {
		...
		case 2:
			var stackFrames = hiss_HissTools.toList(env);
			var g = hiss_HissTools.toDict(this.globals);
			var name = hiss_HissTools.symbolName(exp);
			var v = null;
			var _g = 0;
			while(_g < stackFrames.length) {
				var frameDict = hiss_HissTools.toDict(stackFrames[_g++]);
				if(__map_reserved[name] != null ? frameDict.existsReserved(name) : frameDict.h.hasOwnProperty(name)) {
					v = __map_reserved[name] != null ? frameDict.getReserved(name) : frameDict.h[name];
					break;
				}
			}
			var tmp;
			if(v != null) {
				tmp = v;
			} else if(__map_reserved[name] != null ? g.existsReserved(name) : g.h.hasOwnProperty(name)) {
				tmp = __map_reserved[name] != null ? g.getReserved(name) : g.h[name];
			} else {
				throw new js__$Boot_HaxeError("" + name + " is undefined");
			}
			cc(tmp);
			break;
		case 3:
		...
		case 7:
			...
			this.internalEval(hiss_HissTools.first(exp),env,function(callable) {
				...
				case 9:
					_gthis.funcall(false,exp,env,cc);
					break;
				case 10:
					...
					_gthis.specialForm(args,env1,function(expansion) {
						_gthis1.internalEval(expansion,env1,cc1);
						return;
					});
					break;
				case 11:
					...
					break;
				default:
					throw new js__$Boot_HaxeError("Hiss cannot call " + Std.string(callable) + " from " + hiss_HissTools.toPrint(hiss_HissTools.first(exp)));
				}
				return;
			});
			break;
		...
		case 14:
			...
			case 7:
				...
				tmp2 = hiss_HValue.List(copy);
				break;
			case 13:
				tmp2 = hiss_HValue.Quote(this.evalUnquotes(_g2.exp,env));
				break;
			case 14:
				tmp2 = this.evalUnquotes(_g2.exp,env);
				break;
			case 15:
		...
		case 15:
			...
			break;
		default:
			throw new js__$Boot_HaxeError("Cannot evaluate " + Std.string(exp) + " yet");
		}
	}
	...
	,toNativeFunction: function(fun) {
		...
		if(fun._hx_index == 9) {
			...
			if(_g2 != null && _g2.length == 0) {
				return function() {
					var val = null;
					_gthis.funcall(false,hiss_HValue.List([fun]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),function(_val) {
						val = _val;
						return val;
					});
				...
				return function(arg1) {
					var val1 = null;
					var tmp = hiss_HValue.Quote(hiss_HissTools.toHValue(arg1));
					_gthis.funcall(false,hiss_HValue.List([fun,tmp]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),function(_val1) {
						val1 = _val1;
						return val1;
					});
				...
				return function(arg11,arg2) {
					var val2 = null;
					var tmp1 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg11));
					var tmp2 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg2));
					_gthis.funcall(false,hiss_HValue.List([fun,tmp1,tmp2]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),function(_val2) {
						val2 = _val2;
						return val2;
					});
				...
				return function(arg12,arg21,arg3) {
					var val3 = null;
					var tmp3 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg12));
					var tmp4 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg21));
					var tmp5 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg3));
					_gthis.funcall(false,hiss_HValue.List([fun,tmp3,tmp4,tmp5]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),function(_val3) {
						val3 = _val3;
						return val3;
					});
				...
				return function(arg13,arg22,arg31,arg4) {
					var val4 = null;
					var tmp6 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg13));
					var tmp7 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg22));
					var tmp8 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg31));
					var tmp9 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg4));
					_gthis.funcall(false,hiss_HValue.List([fun,tmp6,tmp7,tmp8,tmp9]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),function(_val4) {
						val4 = _val4;
						return val4;
					});
				...
				return function(arg14,arg23,arg32,arg41,arg5) {
					var val5 = null;
					var tmp10 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg14));
					var tmp11 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg23));
					var tmp12 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg32));
					var tmp13 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg41));
					var tmp14 = hiss_HValue.Quote(hiss_HissTools.toHValue(arg5));
					_gthis.funcall(false,hiss_HValue.List([fun,tmp10,tmp11,tmp12,tmp13,tmp14]),hiss_HValue.List([hiss_HValue.Dict(new haxe_ds_StringMap())]),function(_val5) {
						val5 = _val5;
						return val5;
					});
...
var hiss_HArgType = $hxEnums["hiss.HArgType"] = { __ename__ : "hiss.HArgType", __constructs__ : ["Fixed","Var"]
	,Fixed: {_hx_index:0,__enum__:"hiss.HArgType",toString:$estr}
	,Var: {_hx_index:1,__enum__:"hiss.HArgType",toString:$estr}
};
...
hiss_HaxeTools.shellCommand = function(cmd) {
	throw new js__$Boot_HaxeError("Can't run shell command on non-sys platform.");
};
...
hiss_HissTools.version = function() {
	return hiss_HValue.String("generators-358* (target: js)");
};
...
hiss_HissTools.get = function(dict,key) {
	var _this = hiss_HissTools.toDict(dict);
	if(__map_reserved[key] != null) {
...
hiss_HissTools.range = function(a,b) {
	var intIterator_min;
	var intIterator_max;
	intIterator_min = b == null ? 0 : a;
	intIterator_max = b == null ? a : b;
	return { iterator : function() {
		return { next : function() {
			intIterator_min += 1;
			return hiss_HissTools.toHValue(intIterator_min - 1);
		}, hasNext : function() {
			return intIterator_min < intIterator_max;
		}};
	}};
};
hiss_HissTools.dictExtend = function(dict,extension) {
	var extended = hiss_HissTools.toDict(dict).copy();
	var pair = new haxe_iterators_MapKeyValueIterator(hiss_HissTools.toDict(extension));
	while(pair.hasNext()) {
		var pair1 = pair.next();
		var key = pair1.key;
		var value = pair1.value;
		if(__map_reserved[key] != null) {
			extended.setReserved(key,value);
		} else {
			extended.h[key] = value;
		}
	}
...
hiss_HissTools.destructuringBind = function(names,values) {
	var bindings = hiss_HValue.Dict(new haxe_ds_StringMap());
		...
		_hx_loop1: while(_g1 < _g11) {
			...
			case 2:
				...
				case "&optional":
					...
					bindings = hiss_HissTools.dictExtend(bindings,hiss_HissTools.destructuringBind(hiss_HValue.List(_g.slice(idx + 1)),hiss_HValue.List(remainingValues)));
					break _hx_loop1;
				case "&rest":
				...
				break;
			case 7:
				bindings = hiss_HissTools.dictExtend(bindings,hiss_HissTools.destructuringBind(_g[idx],l2[idx]));
				break;
			default:
...
hiss_HissTools.emptyDict = function() {
	return hiss_HValue.Dict(new haxe_ds_StringMap());
};
hiss_HissTools.eq = function(a,b) {
	if(a._hx_index != b._hx_index) {
		return hiss_HValue.Nil;
	}
	switch(a._hx_index) {
	...
	case 7:
		...
		var i = 0;
		while(i < l1.length) {
			if(!hiss_HissTools.truthy(hiss_HissTools.eq(l1[i],l2[i]))) {
				return hiss_HValue.Nil;
			}
			++i;
		}
	...
	case 13:
		...
		return hiss_HissTools.eq(_g1,bb);
	case 14:
		var _g3 = a.exp;
		var bb1;
		switch(b._hx_index) {
		...
		return hiss_HissTools.eq(_g3,bb1);
	case 15:
		var _g4 = a.exp;
		var bb2;
		switch(b._hx_index) {
		...
		return hiss_HissTools.eq(_g4,bb2);
	case 16:
		var _g = a.exp;
		var bb3;
		switch(b._hx_index) {
		...
		return hiss_HissTools.eq(_g,bb3);
	default:
...
hiss_HissTools.toPrint = function(v,recursiveCall) {
	...
	case 8:
		if(recursiveCall > hiss_HissTools.recursivePrintDepth) {
			...
			var _g1 = [];
			var _g12 = new haxe_iterators_MapKeyValueIterator(v.n);
			while(_g12.hasNext()) {
				var _g21 = _g12.next();
				_g1.push("" + _g21.key + " => " + hiss_HissTools.toPrint(_g21.value,recursiveCall + 1) + ", ");
			}
...
var hiss_StaticFiles = $hx_exports["hiss"]["StaticFiles"] = function() { };
hiss_StaticFiles.__name__ = "hiss.StaticFiles";
hiss_StaticFiles.registerFileContent = function(path,content) {
...
hiss_VariadicFunctions.add = function(args,env,cc) {
	...
	var _g4 = 0;
	var _g5 = hiss_HissTools.unwrapList(args);
	while(_g4 < _g5.length) sum += _g5[_g4++];
...
hiss_VariadicFunctions.multiply = function(args,env,cc) {
	if(hiss_HissTools.$length(args) == 0) {
		cc(hiss_HValue.Int(0));
	} else {
		var first = 1;
		var _g = 0;
