...
var hiss_CCInterp = $hx_exports["hiss"]["CCInterp"] = function(printFunction) {
	...
	this.importFunction(function() {
		return _g8(beginFunction,iterateFunction);
	...
	this.importFunction(function() {
		return _g9(beginFunction1,iterateFunction1);
	...
	this.importFunction(function() {
		return _g10(beginFunction2,iterateFunction2);
	},"enable-continuations");
	var _g11 = $bind(this,this.useBeginAndIterate);
	var beginFunction3 = $bind(this,this.begin);
	var iterateFunction3 = $bind(this,this.iterateCC);
	this.importFunction(function() {
		return _g11(beginFunction3,iterateFunction3);
	...
	this.importFunction(function(dict,key,value) {
	...
	this.importCCFunction(function(args,env,cc) {
		hiss_VariadicFunctions.numCompare(type5,args,env,cc);
	},">");
	var type6 = hiss_Comparison.GreaterEqual;
	this.importCCFunction(function(args,env,cc) {
		hiss_VariadicFunctions.numCompare(type6,args,env,cc);
	},">=");
	var type7 = hiss_Comparison.Equal;
	this.importCCFunction(function(args,env,cc) {
		hiss_VariadicFunctions.numCompare(type7,args,env,cc);
	},"=");
...
hiss_CCInterp.prototype = {
	...
	,importClass: function(clazz,name,methodNameFunction,getterNameFunction,setterNameFunction) {
		...
		while(_g < _g1.length) {
			...
			if(Type.typeof(fieldValue[0])._hx_index == 5) {
				...
				if(this.debugClassImports) {
					haxe_Log.trace(translatedName,{ fileName : "src/hiss/CCInterp.hx", lineNumber : 174, className : "hiss.CCInterp", methodName : "importClass"});
				}
	...
	,repl: function(useConsoleReader) {
		...
		hiss_HaxeTools.println("Hiss version " + "types-452* (target: nodejs)");
	...
	,loop: function(args,env,cc) {
		...
		this.evalAll(hiss_HissTools.alternates(bindings,false),env,function(firstValues) {
			...
			while(true) {
				...
				_gthis.internalEval(hiss_HissTools.cons(hiss_HValue.Symbol("begin"),body),hiss_HissTools.extend(env,hiss_HissTools.destructuringBind(names,_gthis,hiss_HissTools.cons(hiss_HValue.SpecialForm(recur,"recur"),values))),function(value) {
					result = value;
				});
				if(!recurCalled) {
					break;
	...
	,callCC: function(args,env,cc) {
		...
		var ccHFunction = hiss_HValue.Function(function(innerArgs,innerEnv,innerCC) {
			...
			haxe_Log.trace("calling cc#" + ccId + " with " + hiss_HissTools.toPrint(arg),{ fileName : "src/hiss/CCInterp.hx", lineNumber : 962, className : "hiss.CCInterp", methodName : "callCC"});
			cc(arg);
		},"cc");
...
hiss_HaxeTools.shellCommand = function(cmd) {
	...
	while(_g < _g1.length) {
		...
		if($process.error != null) {
			throw haxe_Exception.thrown("child_process error from `" + command + "`: " + Std.string($process.error));
		}
		if($process.status != 0) {
			throw haxe_Exception.thrown("Shell command error from `" + command + "`: " + stringFromChildProcessOutput($process.stderr));
		}
...
hiss_HissTestCase.hissTest = function(interp,args,env,cc) {
	...
	while(_g < _g1.length) {
		...
		try {
			...
			utest_Assert.fail(errorMessage + Std.string(haxe_Exception.caught(_g2).unwrap().toString()),{ fileName : "src/hiss/HissTestCase.hx", lineNumber : 85, className : "hiss.HissTestCase", methodName : "hissTest"});
		}
	}
	var _g = 0;
...
hiss_HissTestCase.prototype = $extend(utest_Test.prototype,{
	...
	,__initializeUtest__: function() {
		...
		init.tests.push({ name : "testWithoutTimeout", dependencies : [], execute : function() {
			_gthis.testWithoutTimeout();
			return utest_Async.getResolved();
		}});
		init.tests.push({ name : "testWithTimeout", dependencies : [], execute : function() {
...
hiss_HissTools.version = function() {
	return hiss_HValue.String("types-452* (target: nodejs)");
};
